...
public class Position
{
	...
	Square? enPassant;
	public Position(Piece? [, ] board, int moves, int halfmoves, Color c, (bool, bool, bool, bool) castling, Square? enPassant)
	{
...
public class Move
{
	...
	private static (int, int)? GetDfferingCoordinate(char c) {
		
	}
	
	public static Move? StringToMove(Position pos, string input){
		Piece? piece_type =  FromChar(input[0]);
		
		if (piece_type is null)
			return null;
		
		Square? end = Square.StringToSquare(input.Substring(input.Length - 2));
		...
		for(int i = 0; i < 8; i++)
			for(int j = 0; j < 8;j++){
				Piece? piece = pos.GetPieceOnSquare(new Square(i, j));
				
				if (piece is null)
					continue;
				
				if(piece.color != pos.colorToMove || piece.type != piece_type.type)
					return null;
...
public class Square : IEquatable<Square>
{
	...
	public int X { get => this.coordinates.Item1; }
	public int Y { get => this.coordinates.Item2; }
	...
	public static (int, int)? ConvertStringToCoordinates(string input)
	{
		...
		bool first_char_condition = (input[0] > 'h' || input[0] < 'a') && (input[0] < 'A' || input[0] > 'H');
...
public class PieceTypeFuncs
{
	...
	public static List<Square> GetTypesPossibleMoves(Square c, PieceType t, Color color = Color.White)
	{
		return t switch
		{
...
public class FEN
{
	...
	{
		return (input.IndexOf('K') >= 0, input.IndexOf('Q') >= 0, input.IndexOf('k') >= 0, input.IndexOf('q') >= 0);
	}
